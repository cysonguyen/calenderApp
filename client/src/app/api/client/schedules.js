...
export async function getSchedulesApi({ userId, dateRange, ...query }) {
    return await axios.get(`${BASE_URL}/schedule/list/${userId}`, { params: { ...dateRange, ...query } });
}

export async function getScheduleByIdApi(scheduleId) {
...
export async function updateScheduleApi({ scheduleId, schedule }) {
